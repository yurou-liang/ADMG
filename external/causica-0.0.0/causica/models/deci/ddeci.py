...
def _param_nan_report(module: torch.nn.Module, max_print: int = 30):
    cnt = 0
    for name, p in module.named_parameters():
        if p is None:
            continue
        if torch.isnan(p).any() or torch.isinf(p).any():
            mn, mx = _safe_minmax(p)
            print(f"[DEBUG] PARAM NaN/Inf: {name:60s} "
                  f"nan={torch.isnan(p).any().item()} inf={torch.isinf(p).any().item()} "
                  f"finite_minmax=({mn},{mx}) shape={tuple(p.shape)}")
            cnt += 1
            if cnt >= max_print:
                break
    if cnt == 0:
        print("[DEBUG] No NaN/Inf found in parameters (yet).")

def _safe_minmax_finite(t: torch.Tensor):
    t = t.detach()
    finite = torch.isfinite(t)
    if finite.any():
        tf = t[finite]
        return tf.min().item(), tf.max().item()
    return None, None

def attach_nan_forward_hooks(root: torch.nn.Module, root_name: str = "root"):
    hooks = []
    ...
    def _hook(mod, inp, out):
        ...
        for o in outs:
            if torch.is_tensor(o) and (torch.isnan(o).any() or torch.isinf(o).any()):
                mn, mx = _safe_minmax_finite(o)
                print(f"[NAN-HOOK] {root_name}.{mod.__class__.__name__} produced NaN/Inf "
                      f"nan={torch.isnan(o).any().item()} inf={torch.isinf(o).any().item()} "
                      f"finite_minmax=({mn},{mx}) shape={tuple(o.shape)}")
                raise RuntimeError("NaN/Inf produced inside forward()")

    for name, m in root.named_modules():
        # hook only "real" modules (skip the root itself if you want)
        hooks.append(m.register_forward_hook(_hook))
    return hooks

def _tensor_has_bad(t):
...
def _any_has_bad(x):
    ...
    if isinstance(x, (list, tuple)):
        return any(_any_has_bad(v) for v in x)
    if isinstance(x, dict):
        return any(_any_has_bad(v) for v in x.values())
    return False

def _safe_minmax_finite(t):
    # works even if t contains NaN/Inf
...
class DDECI(DECI):
    ...
    def _ELBO_terms(self, X: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        def _tstats(name, t):
            ...
            print(f"[DEBUG] {name}: shape={tuple(t.shape)} nan={torch.isnan(t).any().item()} "
                f"inf={torch.isinf(t).any().item()} finite_minmax={mnmx}")

        # print the likely inputs *right before* sampling A_sample
        # (replace these names with the actual tensors in your scope)
        # _tstats("A_logits", A_logits)   # if exists
        # _tstats("A_probs", A_probs)     # if exists
        # _tstats("gumbel_noise", gumbel_noise)  # if exists
        ###########################################
        ...
        ############################################
        if torch.isnan(A_sample).any() or torch.isinf(A_sample).any():
            nan_idx = torch.nonzero(torch.isnan(A_sample), as_tuple=False)
            inf_idx = torch.nonzero(torch.isinf(A_sample), as_tuple=False)

            fin = A_sample[torch.isfinite(A_sample)]
            mnmx = (fin.min().item(), fin.max().item()) if fin.numel() else (None, None)

            print("[DEBUG] A_sample has NaN/Inf!",
                "shape=", tuple(A_sample.shape),
                "dtype=", A_sample.dtype,
                "device=", A_sample.device,
                "finite_minmax=", mnmx)

            print("[DEBUG] A_sample NaNs count:", nan_idx.shape[0])
            print("[DEBUG] first NaN idx:", nan_idx[:20].detach().cpu().tolist())

            print("[DEBUG] A_sample Infs count:", inf_idx.shape[0])
            print("[DEBUG] first Inf idx:", inf_idx[:20].detach().cpu().tolist())

            # print the exact entries you saw (only if 2D)
            if A_sample.dim() == 2 and A_sample.shape[0] > 6 and A_sample.shape[1] > 2:
                print("[DEBUG] A_sample[6,1] =", A_sample[6,1].item(),
                    " A_sample[6,2] =", A_sample[6,2].item())

            print("[DEBUG] ---- locals() tensors with NaN/Inf (excluding A_sample) ----")
            for name, t in list(locals().items()):
                ...
                if torch.is_tensor(t) and (torch.isnan(t).any() or torch.isinf(t).any()):
                    fin = t[torch.isfinite(t)]
                    mnmx = (fin.min().item(), fin.max().item()) if fin.numel() else (None, None)
                    print(f"[DEBUG][LOCAL NaN/Inf] {name}: shape={tuple(t.shape)} dtype={t.dtype} finite_minmax={mnmx}")

            raise RuntimeError("NaN/Inf in A_sample right after sampling")
        ############################################
        ...
        if torch.isnan(W_adj).any() or torch.isinf(W_adj).any():
            print("[DEBUG] W_adj has NaN/Inf. shape:", tuple(W_adj.shape))

            idx = torch.nonzero(torch.isnan(W_adj), as_tuple=False)
            print("[DEBUG] W_adj NaNs count:", idx.shape[0])
            print("[DEBUG] first NaN idx:", idx[:20].detach().cpu().tolist())

            # check operands
            WA = self.ICGNN.get_weighted_adjacency()
            print("[DEBUG] A_sample: nan", torch.isnan(A_sample).any().item(),
                "inf", torch.isinf(A_sample).any().item(),
                "finite min/max", A_sample[torch.isfinite(A_sample)].min().item(), A_sample[torch.isfinite(A_sample)].max().item())
            print("[DEBUG] weighted_adj: nan", torch.isnan(WA).any().item(),
                "inf", torch.isinf(WA).any().item(),
                "finite min/max", WA[torch.isfinite(WA)].min().item(), WA[torch.isfinite(WA)].max().item())

            # print the exact bad entries for your observed case
            if W_adj.dim() == 2 and W_adj.shape[0] > 6 and W_adj.shape[1] > 2:
                print("[DEBUG] W_adj[6,1] =", W_adj[6,1].item(), " W_adj[6,2] =", W_adj[6,2].item())
                print("[DEBUG] A_sample[6,1] =", A_sample[6,1].item(), " A_sample[6,2] =", A_sample[6,2].item())
                print("[DEBUG] WA[6,1] =", WA[6,1].item(), " WA[6,2] =", WA[6,2].item())

            raise RuntimeError("NaN/Inf in W_adj right after construction")
######################################################################
        ...
        x_all = torch.cat([X, U], dim=-1)
        ####################################################
        if torch.isnan(W_adj).any():
            print("[DEBUG] W_adj shape:", tuple(W_adj.shape))

            idx = torch.nonzero(torch.isnan(W_adj), as_tuple=False)
            print("[DEBUG] W_adj NaNs: count =", idx.shape[0])
            print("[DEBUG] first 20 NaN idx in W_adj:", idx[:20].detach().cpu().tolist())

            # show the entire lag slice that contains NaNs
            bad_lag = int(idx[0, 0].item())
            print("[DEBUG] bad_lag =", bad_lag)

            # print the two bad "rows" (interpretation depends on shape)
            if W_adj.dim() == 3:
                # W_adj[lag, i, j]
                bad_is = sorted(set(idx[:,1].detach().cpu().tolist()))
                for i in bad_is[:5]:
                    row = W_adj[bad_lag, i, :]
                    fin = row[torch.isfinite(row)]
                    print(f"[DEBUG] W_adj[{bad_lag},{i},:] nan={torch.isnan(row).any().item()} "
                        f"finite_minmax={(fin.min().item(), fin.max().item()) if fin.numel() else (None,None)}")
            elif W_adj.dim() == 2:
                # W_adj[lag, i]
                bad_is = sorted(set(idx[:,1].detach().cpu().tolist()))
                for i in bad_is[:10]:
                    print(f"[DEBUG] W_adj[{bad_lag},{i}] =", W_adj[bad_lag, i].item())

            raise RuntimeError("NaN in W_adj")
         ####################################################
        predict = self.ICGNN.predict(x_all, W_adj)

        intervention_mask = torch.zeros(self.processed_dim_all, device=self.device, dtype=torch.bool)
        ##############################################
        # --- DEBUG: NaN locator (remove later) ---
        
        if torch.isnan(X).any() or torch.isinf(X).any():
            mn, mx = _safe_minmax(X)
            print("[DEBUG] NaN/Inf in X",
                "nan", torch.isnan(X).any().item(),
                "inf", torch.isinf(X).any().item(),
                "min/max", mn, mx)
            raise RuntimeError("NaN/Inf in X before inference network")

        if torch.isnan(U).any() or torch.isinf(U).any():
            mn, mx = _safe_minmax(U)
            print("[DEBUG] NaN/Inf in sampled U",
                "nan", torch.isnan(U).any().item(),
                "inf", torch.isinf(U).any().item(),
                "min/max", mn, mx)
            raise RuntimeError("NaN/Inf in U = qu_x.rsample()")

        if torch.isnan(x_all).any() or torch.isinf(x_all).any():
            print("[DEBUG] NaN/Inf in x_all",
                "nan", torch.isnan(x_all).any().item(),
                "inf", torch.isinf(x_all).any().item())
            raise RuntimeError("NaN/Inf in x_all")

        if torch.isnan(predict).any() or torch.isinf(predict).any():
            mn, mx = _safe_minmax(predict)
            print("[DEBUG] NaN/Inf in predict",
                "nan", torch.isnan(predict).any().item(),
                "inf", torch.isinf(predict).any().item(),
                "predict min/max (finite)", mn, mx)

            bad_cols = torch.isnan(predict).any(0).nonzero(as_tuple=False).flatten().tolist()
            print("[DEBUG] bad predict cols:", bad_cols[:50])

            # NEW: check whether weights already blew up
            _param_nan_report(self)
            # right before raising for NaN predict:
            # print("[DEBUG] Re-running forward once with NaN hooks to locate first bad layer...")

            # hooks = []
            # try:
            #     # attach hooks to the decoder/SEM part that produces `predict`
            #     # YOU need to choose the module that outputs predict. Usually one of these exists:
            #     #   self.icgnn / self.sem / self.decoder / self.f / self.g
            #     # Start with the most likely one: the module that produced `predict`.
            #     hooks = attach_nan_forward_hooks(self.ICGNN, root_name="ICGNN")  # <-- adjust name if different

            #     # re-run the same forward that creates `predict`
            #     # put *exactly the same call you used to compute predict*
            #     _ = self.ICGNN(x_all,  W_adj)  # <-- replace with the real call used in your code

            # finally:
            #     for h in hooks:
            #         h.remove()

            print("[DEBUG] Re-running ICGNN.predict with NaN hooks to locate first bad module...")
            ...
            def _count_bad(t):
                ...
                fin = torch.isfinite(t).sum().item()
                return nan, inf, fin, t.numel()

            def hook_fn(name):
                def _hook(module, inp, out):
                    # inp is a tuple
                    bad_in = _any_has_bad(inp)
                    bad_out = _any_has_bad(out)

                    if bad_in or bad_out:
                        print(f"[DEBUG][HIT] {name} ({type(module).__name__}) bad_in={bad_in} bad_out={bad_out}")

                        # print input stats (first tensor-like input)
                        for k, v in enumerate(inp):
                            if torch.is_tensor(v):
                                nan, inf, fin, tot = _count_bad(v)
                                mn, mx = _safe_minmax_finite(v)
                                print(f"  inp[{k}] shape={tuple(v.shape)} nan/inf/fin/tot={nan}/{inf}/{fin}/{tot} finite_minmax={mn},{mx}")
                                break

                        # print output stats
                        if torch.is_tensor(out):
                            nan, inf, fin, tot = _count_bad(out)
                            mn, mx = _safe_minmax_finite(out)
                            print(f"  out shape={tuple(out.shape)} nan/inf/fin/tot={nan}/{inf}/{fin}/{tot} finite_minmax={mn},{mx}")
                        else:
                            print(f"  out type={type(out)} (non-tensor; contains NaN/Inf somewhere)")

                        raise RuntimeError(f"[DEBUG] First bad (NaN/Inf) seen at: {name}")
                return _hook

            # Register hooks on everything inside ICGNN (skip the top-level container if you want; harmless either way)
            for n, m in self.ICGNN.named_modules():
                # usually helpful to skip empty name which is the root module
                if n == "":
                    continue
                hooks.append(m.register_forward_hook(hook_fn(f"ICGNN.{n}")))

            try:
                # IMPORTANT: rerun the EXACT same call
                _ = self.ICGNN.predict(x_all, W_adj)
            finally:
                for h in hooks:
                    h.remove()

            raise RuntimeError("NaN/Inf in predict (before log_prob)")
        # --- END DEBUG ---
###########################################################################################
