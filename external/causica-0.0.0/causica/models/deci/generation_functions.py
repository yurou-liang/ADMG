...
class FGNNI(nn.Module):
    ...
    def feed_forward(self, X: torch.Tensor, W_adj: torch.Tensor) -> torch.Tensor:
        ...
        # Run f
        # --- DEBUG: locate NaNs in X_in_f (remove later) ---
        ###########################################################################
       # works on older torch
        nan_mask = torch.isnan(X_in_f)
        inf_mask = torch.isinf(X_in_f)

        nan_cols = nan_mask.any(dim=0).any(dim=0).nonzero(as_tuple=False).flatten().tolist()
        inf_cols = inf_mask.any(dim=0).any(dim=0).nonzero(as_tuple=False).flatten().tolist()

        # print("[DEBUG] X_in_f has NaN/Inf nan_cols=", nan_cols, "inf_cols=", inf_cols)
        # print("[DEBUG] NaN count per last-dim feature:", nan_mask.sum(dim=0).sum(dim=0).cpu().tolist())

        if nan_cols or inf_cols:
            # Where exactly are the NaNs?
            nan_idx = torch.nonzero(nan_mask, as_tuple=False)  # (row: [batch, node, feat])
            print("[DEBUG] first 20 NaN indices (batch,node,feat):", nan_idx[:20].cpu().tolist())

            # Which nodes/features are affected?
            bad_nodes = torch.unique(nan_idx[:, 1]).cpu().tolist()
            bad_feats = torch.unique(nan_idx[:, 2]).cpu().tolist()
            print("[DEBUG] bad_nodes:", bad_nodes)
            print("[DEBUG] bad_feats:", bad_feats)

            # Now check whether the inputs to feed_forward are already broken
            X = X  # whatever the argument name is in your function signature
            print("[DEBUG] X passed into feed_forward: nan", torch.isnan(X).any().item(),
                "inf", torch.isinf(X).any().item(),
                "finite_min/max", X[torch.isfinite(X)].min().item(), X[torch.isfinite(X)].max().item())

            print("[DEBUG] W_adj passed into feed_forward: nan", torch.isnan(W_adj).any().item(),
                "inf", torch.isinf(W_adj).any().item(),
                "finite_min/max", W_adj[torch.isfinite(W_adj)].min().item(), W_adj[torch.isfinite(W_adj)].max().item())

            raise RuntimeError("NaN/Inf detected in X_in_f before first Linear")
