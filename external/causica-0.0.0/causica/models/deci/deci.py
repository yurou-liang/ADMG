...
class DECI(
    ...
    def get_adj_matrix_tensor(
        ...
        ################################################
        # --- DEBUG: where NaNs are introduced (remove later) ---
        raw_adj = adj
        if torch.isnan(raw_adj).any():
            idx = torch.nonzero(torch.isnan(raw_adj), as_tuple=False)
            print("[DEBUG] NaN already in raw adj BEFORE constraints. count=", idx.shape[0],
                "first idx=", idx[:20].detach().cpu().tolist())
        # apply constraints
        constr_adj = self._apply_constraints(raw_adj)

        if torch.isnan(constr_adj).any():
            idx2 = torch.nonzero(torch.isnan(constr_adj), as_tuple=False)
            print("[DEBUG] NaN appears AFTER constraints. count=", idx2.shape[0],
                "first idx=", idx2[:20].detach().cpu().tolist())

            # also inspect constraint matrices at those positions
            s0, i0, j0 = idx2[0].tolist()
            print("[DEBUG] first NaN at sample,i,j =", s0, i0, j0)
            print("[DEBUG] Example NaN position (s,i,j)=", (i0, j0))
            # NOTE: constr_adj has shape (samples, n, n)
            s, i, j = idx2[0].tolist()
            print("[DEBUG] raw_adj[s,i,j]=", raw_adj[s,i,j].item())
            print("[DEBUG] neg_constraint[i,j]=", self.neg_constraint_matrix[i,j].item(),
                "pos_constraint[i,j]=", self.pos_constraint_matrix[i,j].item())

            # check if constraint matrices themselves have NaNs
            if torch.isnan(self.neg_constraint_matrix).any() or torch.isnan(self.pos_constraint_matrix).any():
                print("[DEBUG] constraint matrices contain NaNs!",
                    "neg_nan=", torch.isnan(self.neg_constraint_matrix).any().item(),
                    "pos_nan=", torch.isnan(self.pos_constraint_matrix).any().item())

            raise RuntimeError("[DEBUG] NaN in adj after constraints")
        # --- END DEBUG ---
        return constr_adj
    ...
    def _log_prob(
        ...
        **_,
        ...
        continuous_range = [i for region in typed_regions["continuous"] for i in region]
        if continuous_range:
            z = x[..., continuous_range] - predict[..., continuous_range]
##############################################################################
            if torch.isnan(predict).any(): raise RuntimeError("NaN in predict (decoder output)")
            if torch.isnan(x[..., continuous_range]).any():
                raise RuntimeError("NaN in x continuous slice")
            if torch.isnan(predict[..., continuous_range]).any():
                raise RuntimeError("NaN in predict continuous slice")
            if torch.isnan(z).any():
                # super useful debug
                raise RuntimeError(
                    f"NaN in z=x-predict. "
                    f"x min/max={x[...,continuous_range].min().item():.3g}/{x[...,continuous_range].max().item():.3g}, "
                    f"pred min/max={predict[...,continuous_range].min().item():.3g}/{predict[...,continuous_range].max().item():.3g}"
                )

            # Check continuous likelihood params (GaussianBase)
            gb = self.likelihoods["continuous"]
            # gb.logscale_base exists for GaussianBase
            if hasattr(gb, "logscale_base"):
                if torch.isnan(gb.logscale_base).any() or torch.isinf(gb.logscale_base).any():
                    raise RuntimeError(f"NaN/Inf in GaussianBase.logscale_base: {gb.logscale_base}")

                scale = torch.exp(gb.logscale_base)
                if torch.isnan(scale).any() or torch.isinf(scale).any() or (scale <= 0).any():
                    raise RuntimeError(f"Invalid GaussianBase scale: {scale}")
        ##############################################################################
